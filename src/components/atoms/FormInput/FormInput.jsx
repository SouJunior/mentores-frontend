...
const schema = yup
	.object({
		...
		senha: yup.string().max(8).required('senha inválida'),
		Confsenha: yup
			.string()
		...
		<main className={styles.layout}>
			<div className={styles.boxLogin}>
				<form
					...
					{errors.data && (
						<span className={styles.error}>{errors.data?.message}</span>
					)}
					<p className={styles.nomes}>
					...
					<FormEmailRevalidation
						{...register('Confemail', { required: true })}
					/>
					{errors.Confemail && (
						<span className={styles.error}>{errors.Confemail?.message}</span>
					)}
					<p className={styles.nomes}>
					...
					<FormPasswordRevalidation
						{...register('Confsenha', { required: true })}
					/>
					{errors.Confsenha && (
						<span className={styles.error}>{errors.Confsenha?.message}</span>
					)}
					<div className={styles.check}>
						<input type='checkbox'></input>
						<span className={styles.termo}>
							Concordo com os{' '}
							<button
								type='button'
