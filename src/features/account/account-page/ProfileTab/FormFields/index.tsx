import EditPhotoModal from '@/features/account/edit-photo-modal';
import { Modal } from '@/components/modal';
import PhotoButton from '@/features/account/photo-button';
...
export function FormFields() {
  ...
  return (
    <div className="grid grid-cols-[8rem_1fr] items-center gap-y-12 gap-x-16">
        ...
        <Modal.Control asChild>
          ...
          >
            ...
            <svg
              ...
              viewBox="0 0 40 40"
              fill="none"
              className="camera-icon text-[#666666] w-10 h-10 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[1]"
            >
              <path
      ...
      <div>
        ...
        <div className="grid grid-cols-[repeat(3,9.5rem)] gap-4 mt-4">
          {specialtiesOptions.map(specialty => (
            <div
              key={specialty}
              className={`flex items-center justify-center gap-0.5 py-2 px-3 h-[2.2rem] rounded-[50px] text-center cursor-pointer [&_svg]:${formik.values.specialties?.includes(specialty) ?? mentor.data?.specialties.includes(specialty) ? 'block' : 'hidden'} ${
          ...
          <span className="text-[#323232] leading-[1.2rem] [&_strong]:font-semibold">
      ...
      <div aria-hidden />
      <section className="[&_label_&_span:first-child]:text-[#323232] [&_label_&_span:first-child_.asterisk]:text-[#338AFF] [&_p]:text-right [&_p]:mt-2">
        <InputForm
        ...
        <p
          className={`text-[0.875rem] leading-4 ${isMaxCharactersExceeded ? 'text-[#E94242]' : 'text-[#323232]'}`}
        >
          {handleMaxCharacters()}
        </p>
      </section>
